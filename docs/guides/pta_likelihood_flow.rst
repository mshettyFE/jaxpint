...
For synthetic data generation, JaxPINT has native counterparts of PINT's simulation helpers -- no PINT install needed: :func:`jaxpint.simulation.make_fake_toas_uniform` (and :func:`~jaxpint.simulation.make_fake_toas_from_mjds` for a custom cadence) take a parsed ``.par`` and return :class:`~jaxpint.types.TOAData` whose timestamps realize the timing model to better than a nanosecond, with optional white or correlated noise on top. The barycentre pseudo-observatory (``obs="@"``) works too. PINT's own `pint.simulation <https://nanograv-pint.readthedocs.io/en/latest/_autosummary/pint.simulation.html>`_ generators remain usable through the bridge (``jaxpint[pint]`` extra) and produce TOAs the two implementations agree on at the few-nanosecond level.
