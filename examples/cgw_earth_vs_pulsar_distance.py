...
def main():
    ...
    print(f"Earth-term  median D_L = {np.median(fe):.1f} Mpc, "
          f"R_eff = {float(bs['r_eff_mpc']):.2f} Mpc")
    print(f"Pulsar-term median D_L = {np.median(fp):.1f} Mpc, "
    ...
    def _overlay_pulsars(anchor_color="red", other_color="white"):
        ...
        if is_anchor.any():
            hp.projscatter(psr_lon[is_anchor], psr_lat[is_anchor], lonlat=True,
                           s=45, c=anchor_color, edgecolor="black",
                           linewidths=0.5, marker="*")
        if (~is_anchor).any():
            hp.projscatter(psr_lon[~is_anchor], psr_lat[~is_anchor], lonlat=True,
                           s=18, c=other_color, edgecolor="black",
    ...
    for i, (m, title) in enumerate(
        ...
        hp.mollview(
            ...
            unit=r"$D_L$ [Mpc]", cmap="viridis", norm="log",
            min=float(finite.min()), max=float(finite.max()),
            sub=(1, 2, i + 1), fig=fig.number,
        )
        hp.graticule(dpar=30, dmer=30, color="gray", alpha=0.3)
        _overlay_pulsars()
    fig.suptitle(f"CGW distance sensitivity, nside={nside} "
    ...
    # ---- 2. Per-pixel ratio ---------------------------------------------------
    ratio = np.where(np.isfinite(d_e) & np.isfinite(d_p) & (d_e > 0),
                     d_p / d_e, np.nan)
    fin_r = ratio[np.isfinite(ratio)]
    fig = plt.figure(figsize=(8, 5))
    hp.mollview(
        np.where(np.isfinite(ratio), ratio, hp.UNSEEN),
        title=(r"Pulsar-term distance boost ($D_L^{\rm PT}/D_L^{\rm ET}$)" "\n"
               f"median = {np.median(fin_r):.3f}x, "
               f"range {fin_r.min():.3f}x to {fin_r.max():.3f}x"),
        unit=r"$D_L^{\rm PT}/D_L^{\rm ET}$", cmap="magma", fig=fig.number,
    )
    hp.graticule(dpar=30, dmer=30, color="gray", alpha=0.3)
    ...
    bins = np.logspace(np.log10(min(fe.min(), fp.min())),
                       np.log10(max(fe.max(), fp.max())), 50)
    ax.hist(fe, bins=bins, alpha=0.55, color="C0",
            label=f"Earth-term only (median {np.median(fe):.1f} Mpc)")
    ax.hist(fp, bins=bins, alpha=0.55, color="C3",
