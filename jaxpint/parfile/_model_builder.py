...
Replicates the logic of ``jaxpint.bridge.component_builder`` but reads
from a :class:`ParResult` (produced by the standalone .par parser)
instead of PINT objects.
"""
...
def build_model(
    ...
    # Astrometry info shared with Shapiro / binary
    _astro_raj = "RAJ"
    _astro_decj = "DECJ"
    _astro_pmra = None
    _astro_pmdec = None
    ...
    # ---- Spindown ----
    if Component.SPINDOWN in comp_set:
        spin_names = ["F0"]
        # Collect F1, F2, ... from ParameterVector
        for pname in par.params.names:
    ...
    # ---- Glitch ----
    if Component.GLITCH in comp_set:
    ...
    if Component.ASTROMETRY_EQUATORIAL in comp_set:
        ...
        if _astro_pmra is not None or _astro_pmdec is not None:
            _astro_posepoch = "POSEPOCH" if _has_param(par, "POSEPOCH") else "PEPOCH"
    ...
    # ---- Astrometry Ecliptic ----
    if Component.ASTROMETRY_ECLIPTIC in comp_set:
    ...
    # ---- DispersionDM ----
    if Component.DISPERSION_DM in comp_set:
        dm_names = ["DM"]
        for pname in par.params.names:
            if pname.startswith("DM") and pname != "DM" and pname != "DMEPOCH":
                # Check it's a DM Taylor term (DM1, DM2, ...) not DMX etc.
                suffix = pname[2:]
                if suffix.isdigit():
    ...
    # ---- DispersionDMX ----
    if Component.DISPERSION_DMX in comp_set:
    ...
    # ---- DispersionJump ----
    if Component.DISPERSION_JUMP in comp_set:
        dmjump_names = tuple(
    ...
    # ---- Binary ----
    if par.binary_model is not None:
        _astro = {
            "raj_name": _astro_raj,
            "decj_name": _astro_decj,
            "pmra_name": _astro_pmra,
    ...
    # ---- SolarSystemShapiro ----
    # Auto-add when any astrometry is present
    if Component.ASTROMETRY_EQUATORIAL in comp_set or Component.ASTROMETRY_ECLIPTIC in comp_set:
    ...
    # ---- SolarWindDispersion ----
    if Component.SOLAR_WIND_DISPERSION in comp_set:
        ne_sw_names = ["NE_SW"]
        for pname in par.params.names:
    ...
    # ---- PhaseJump ----
    if Component.PHASE_JUMP in comp_set:
    ...
    # ---- WaveX ----
    if Component.WAVE_X in comp_set:
    ...
    # ---- DMWaveX ----
    if Component.DM_WAVE_X in comp_set:
        dmwx_indices = _collect_prefix_indices(par, "DMWXFREQ_")
        if dmwx_indices:
            dmwxepoch_name = "DMWXEPOCH" if _has_param(par, "DMWXEPOCH") else "PEPOCH"
    ...
    # ---- CMWaveX ----
    if Component.CM_WAVE_X in comp_set:
        cmwx_indices = _collect_prefix_indices(par, "CMWXFREQ_")
        if cmwx_indices:
            cmwxepoch_name = "CMWXEPOCH" if _has_param(par, "CMWXEPOCH") else "PEPOCH"
    ...
    # ---- ChromaticCM ----
    if Component.CHROMATIC_CM in comp_set:
        cm_names = ["CM"]
        for pname in par.params.names:
    ...
    # ---- ChromaticCMX ----
    if Component.CHROMATIC_CMX in comp_set:
    ...
    # ---- FrequencyDependent ----
    if Component.FREQUENCY_DEPENDENT in comp_set:
        fd_names = sorted(
            (n for n in par.params.names if re.match(r"^FD\d+$", n)),
            key=lambda n: int(n[2:]),
        )
        if fd_names:
            delay_components.append(FrequencyDependent(fd_param_names=tuple(fd_names)))

    # ---- FDJump ----
    if Component.FD_JUMP in comp_set:
        fdjump_names = []
        fdjump_indices = []
        for pname in par.params.names:
    ...
    # ---- ExponentialDip ----
    if Component.EXPONENTIAL_DIP in comp_set:
    ...
    # ---- PiecewiseSpindown ----
    if Component.PIECEWISE_SPINDOWN in comp_set:
    ...
    # ---- Wave ----
    if Component.WAVE in comp_set:
        wave_a_names = sorted(
            (n for n in par.params.names if re.match(r"^WAVE\d+_A$", n)),
            key=lambda n: int(re.match(r"WAVE(\d+)_A", n).group(1)),
        )
        if wave_a_names:
            wave_indices = [
                int(re.match(r"WAVE(\d+)_A", n).group(1)) for n in wave_a_names
            ]
    ...
    # ---- IFunc ----
    if Component.IFUNC in comp_set:
        ifunc_a_names = sorted(
            (n for n in par.params.names if re.match(r"^IFUNC\d+_A$", n)),
            key=lambda n: int(re.match(r"IFUNC(\d+)_A", n).group(1)),
        )
        if ifunc_a_names:
    ...
    # ---- Noise: ScaleToaError ----
    if Component.SCALE_TOA_ERROR in comp_set:
    ...
    # ---- Noise: ScaleDmError ----
    if Component.SCALE_DM_ERROR in comp_set:
        dmefac_names = tuple(sorted(n for n in par.params.names if n.startswith("DMEFAC")))
        dmequad_names = tuple(sorted(n for n in par.params.names if n.startswith("DMEQUAD")))
    ...
    # ---- Noise: EcorrNoise ----
    if Component.ECORR_NOISE in comp_set:
        ...
        if toa_data is not None and len(ecorr_names) > 0:
            ...
            for ename in ecorr_names:
                if hasattr(toa_data, "flag_masks") and toa_data.flag_masks is not None:
                    mask = np.asarray(toa_data.flag_masks.get(ename, np.zeros(len(tdb_s), dtype=bool)))
                else:
    ...
    # ---- Noise: PLRedNoise ----
    if Component.PL_RED_NOISE in comp_set and toa_data is not None:
        tdb_s = (
            np.asarray(toa_data.tdb_int) * 86400.0
            + np.asarray(toa_data.tdb_frac) * 86400.0
        )
        ...
        # Check for TNREDTSPAN
        if _has_param(par, "TNREDTSPAN"):
            tspan_days = float(par.params.values[par.params._name_to_index["TNREDTSPAN"]])
    ...
    # ---- Noise: PLDMNoise ----
    if Component.PL_DM_NOISE in comp_set and toa_data is not None:
        tdb_s = (
            np.asarray(toa_data.tdb_int) * 86400.0
            + np.asarray(toa_data.tdb_frac) * 86400.0
        )
        ...
        if _has_param(par, "TNDMTSPAN"):
            tspan_days = float(par.params.values[par.params._name_to_index["TNDMTSPAN"]])
            T = tspan_days * 86400.0
    ...
    # ---- Noise: PLChromNoise ----
    if Component.PL_CHROM_NOISE in comp_set and toa_data is not None:
        tdb_s = (
            np.asarray(toa_data.tdb_int) * 86400.0
            + np.asarray(toa_data.tdb_frac) * 86400.0
        )
        ...
        if _has_param(par, "TNCHROMTSPAN"):
            tspan_days = float(par.params.values[par.params._name_to_index["TNCHROMTSPAN"]])
            T = tspan_days * 86400.0
    ...
    # ---- Noise: PLSWNoise ----
    if Component.PL_SW_NOISE in comp_set and toa_data is not None:
        tdb_s = (
            np.asarray(toa_data.tdb_int) * 86400.0
            + np.asarray(toa_data.tdb_frac) * 86400.0
        )
