"""White noise model for JaxPINT.

Implements EFAC/EQUAD scaling of TOA uncertainties, matching PINT's
``ScaleToaError`` component.  The scaled uncertainties are used as
diagonal weights in the WLS fitter.

Mathematical model (per-TOA)::

    σ_eff = EFAC × √(σ_raw² + EQUAD²)

Each EFAC/EQUAD applies to a subset of TOAs identified by a boolean
