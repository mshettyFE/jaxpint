...
from jaxpint.pta.detection import (
    quadrature_blocks, fstat_skymap, fstat_p, fstat_p_pvalue, phase_shift_background,
    sky_scramble_background, pvalue, _antenna_grid,
)
from tests.helpers import make_simple_pulsar

LOG10_FGW = -5.0  # resolved over the synthetic span -> full-rank network Gram
_POSITIONS = [
    jnp.array([0.2, 0.5, -0.84]), jnp.array([-0.6, 0.3, 0.7]), jnp.array([0.8, -0.5, 0.1]),
]
...
def real_blocks():
    ...
    for i, p in enumerate(_POSITIONS):
        ...
        pulsars.append((g, skel, td, pos)); sc_l.append(sc); gram_l.append(gram)
    return {
        "pulsars": pulsars,
...
def test_factorized_2f_matches_fstat(real_blocks):
    ...
    mine = fstat_skymap(real_blocks["sc_all"], real_blocks["gram_all"], positions, ct, gp)

    direct = []
    for c, p in zip(ct.tolist(), gp.tolist()):
        M, b = jnp.zeros((4, 4)), jnp.zeros(4)
        for g, skel, td, pos in pulsars:
            def logL(amp, cos_inc, psi, phase0, g=g, skel=skel, td=td, pos=pos, c=c, p=p):
                cw = jnp.array([amp, c, p, LOG10_FGW, cos_inc, psi, phase0])
                d = cw_delay_from_array(td, pos, 1.0, cw, earth_term_only=True, linear_amplitude=True)
...
def test_backgrounds_reject_a_coherent_signal():
    ...
    gram_all = jax.vmap(lambda quad: quad @ quad.T + jnp.eye(2))(L)  # SPD quadrature Grams
    c_true = jax.random.normal(kc, (4,))

    F = _antenna_grid(positions, jnp.array([ct_truth]), jnp.array([gp_truth]))[0]  # (npsr, 2)
    ab = jnp.stack(  # (alpha, beta)_a = (fp c1 + fc c3, fp c2 + fc c4)
        [F[:, 0] * c_true[0] + F[:, 1] * c_true[2],
         F[:, 0] * c_true[1] + F[:, 1] * c_true[3]], axis=1)
    ...
    stat = float(m.max())
    bp = phase_shift_background(sc_all, gram_all, positions, ct, gp, 500, jax.random.PRNGKey(1))
...
def test_fstat_p_null_is_chi2_2n():
    ...
    gram_all = jax.vmap(lambda quad: quad @ quad.T + jnp.eye(2))(jax.random.normal(kg, (npsr, 2, 2)))
