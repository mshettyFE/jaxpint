...
from tests.helpers import enterprise_golden, make_fourier_basis, make_simple_pulsar
...
# Frozen golden data lives in tests/data/enterprise_goldens.json — generated
# by enterprise itself via tools/gen_enterprise_goldens.py and read through
# helpers.enterprise_golden; nothing recomputes it at test time.
...
TURNOVER_SHAPE = {"lf0": -8.5, "kappa": 10.0 / 3.0, "beta": 0.5}
TURNOVER_KNEE_SHAPE = {
    "lfb": -8.65,
    "lfk": -7.5,
    "kappa": 10.0 / 3.0,
...
def test_powerlaw_matches_frozen_enterprise_golden():
    """psd_weights == the frozen output of enterprise's gp_priors.powerlaw
    (cached parity, see test_turnover_matches_frozen_enterprise_golden).

    Complementary to the paper-transcription reference above: that test
    anchors the normalization to the published equation, this one to
    enterprise's implementation — the two authorities are independent.
    """
    spec = PowerLawSpectrum(log10_A=LOG10_A, gamma=GAMMA)
    w = np.asarray(spec.psd_weights(FREQS, DF, _value_of(spec.param_defaults())))
...
def test_turnover_matches_frozen_enterprise_golden():
    """psd_weights == the frozen output of enterprise's gp_priors.turnover.

    A cached parity test: the golden vector is enterprise's own numbers
    (tests/data/enterprise_goldens.json, regenerated by
    tools/gen_enterprise_goldens.py), so there is no in-repo transcription
    of the formula to share a misreading with the implementation — and it
    runs without enterprise installed.  The params/grid passed to
    ``enterprise_golden`` are asserted against the golden's generation
    record, and the live check that the frozen values track current
    enterprise is tests/enterprise_checks/test_ent_building_blocks.py.
    """
    spec = TurnoverSpectrum(log10_A=LOG10_A, gamma=GAMMA, **TURNOVER_SHAPE)
    w = np.asarray(spec.psd_weights(FREQS, DF, _value_of(spec.param_defaults())))
    ref = enterprise_golden(
        "turnover_weights",
        params={"log10_A": LOG10_A, "gamma": GAMMA, **TURNOVER_SHAPE},
        grid={"t_span_s": T_SPAN, "n_components": N_COMP},
    )
    npt.assert_allclose(w, ref, rtol=1e-12)


def test_turnover_reduces_to_powerlaw():
    """With the turnover far below the band, the suppression factor is 1."""
    spec = TurnoverSpectrum(log10_A=LOG10_A, gamma=GAMMA, lf0=-12.0)
    w = spec.psd_weights(FREQS, DF, _value_of(spec.param_defaults()))
    ref = _powerlaw_weights_reference(FREQS, LOG10_A, GAMMA, DF)
    npt.assert_allclose(np.asarray(w), ref, rtol=1e-12)


def test_turnover_suppresses_below_f0():
    """With f_0 above the whole band, each bin is suppressed by exactly
    (1 + (f_0/f)^κ)^(2β) relative to the plain power law."""
    lf0 = float(jnp.log10(FREQS[-1])) + 1.0  # turnover above the band
    kappa, beta = 10.0 / 3.0, 0.5
    spec = TurnoverSpectrum(
        log10_A=LOG10_A, gamma=GAMMA, lf0=lf0, kappa=kappa, beta=beta
    )
    w = np.asarray(spec.psd_weights(FREQS, DF, _value_of(spec.param_defaults())))[0::2]
    pl = _powerlaw_weights_reference(FREQS, LOG10_A, GAMMA, DF)[0::2]
    expected = (1.0 + (10.0**lf0 / np.asarray(FREQS)) ** kappa) ** (-2.0 * beta)
    npt.assert_allclose(w / pl, expected, rtol=1e-10)
    assert (w < pl).all()


def test_turnover_knee_matches_frozen_enterprise_golden():
    """psd_weights == the frozen output of enterprise's gp_priors.turnover_knee
    (cached parity, see test_turnover_matches_frozen_enterprise_golden)."""
    spec = TurnoverKneeSpectrum(log10_A=LOG10_A, gamma=GAMMA, **TURNOVER_KNEE_SHAPE)
    w = np.asarray(spec.psd_weights(FREQS, DF, _value_of(spec.param_defaults())))
    ref = enterprise_golden(
        "turnover_knee_weights",
        params={"log10_A": LOG10_A, "gamma": GAMMA, **TURNOVER_KNEE_SHAPE},
        grid={"t_span_s": T_SPAN, "n_components": N_COMP},
    )
    npt.assert_allclose(w, ref, rtol=1e-12)


def test_turnover_knee_reduces_to_powerlaw():
    """Bend far below the band and δ = 0 turn both factors off."""
    spec = TurnoverKneeSpectrum(
        log10_A=LOG10_A, gamma=GAMMA, lfb=-12.0, lfk=-7.5, delta=0.0
    )
    w = spec.psd_weights(FREQS, DF, _value_of(spec.param_defaults()))
    ref = _powerlaw_weights_reference(FREQS, LOG10_A, GAMMA, DF)
    npt.assert_allclose(np.asarray(w), ref, rtol=1e-12)
...
def test_turnover_knee_steepens_above_knee():
    """With the knee below the band and no low-frequency bend, each bin is
    suppressed by exactly (1 + f/f_k)^(2δ) (δ < 0)."""
    lfk = float(jnp.log10(FREQS[0])) - 1.0  # knee below the whole band
    delta = -1.0
    spec = TurnoverKneeSpectrum(
        log10_A=LOG10_A, gamma=GAMMA, lfb=-12.0, lfk=lfk, delta=delta
    )
    w = np.asarray(spec.psd_weights(FREQS, DF, _value_of(spec.param_defaults())))[0::2]
    pl = _powerlaw_weights_reference(FREQS, LOG10_A, GAMMA, DF)[0::2]
    expected = (1.0 + np.asarray(FREQS) / 10.0**lfk) ** (2.0 * delta)
    npt.assert_allclose(w / pl, expected, rtol=1e-10)
    assert (w < pl).all()
...
def test_free_spectrum_logL_matches_matched_powerlaw(correlated):
    r"""pta_logL(free spectrum @ matched ρ) == pta_logL(power law).
...
@pytest.mark.parametrize("knee", [False, True], ids=["turnover", "turnover_knee"])
def test_turnover_logL_matches_neutralized_powerlaw(knee):
    """pta_logL(turnover @ neutral shape) == pta_logL(power law).

    The shape parameters are pushed to values where the turnover/knee
    factors are 1 (bend far below the band, δ = 0), so the CURN path must
    reproduce the power-law likelihood exactly — certifying the injector
    registration + ``value_of`` wiring for the new multi-parameter spectra.
    """
    if knee:
        spec = TurnoverKneeSpectrum(
            log10_A=LOG10_A, gamma=GAMMA, lfb=-13.0, lfk=-7.5, delta=0.0
        )
    else:
        spec = TurnoverSpectrum(log10_A=LOG10_A, gamma=GAMMA, lf0=-13.0)
    pl = CURNInjector(
        N_COMP, T_SPAN, initial_values={"log10_A": LOG10_A, "gamma": GAMMA}
    )
    to = CURNInjector(N_COMP, T_SPAN, spectrum=spec)
    gp_pl, pps, config_pl = _config_and_params(pl, correlated=False)
    gp_to, _, config_to = _config_and_params(to, correlated=False)

    logL_pl = float(pta_logL(gp_pl, pps, config_pl))
    logL_to = float(pta_logL(gp_to, pps, config_to))
    assert np.isfinite(logL_pl)
    npt.assert_allclose(logL_to, logL_pl, rtol=1e-12)


@pytest.mark.parametrize("knee", [False, True], ids=["turnover", "turnover_knee"])
def test_turnover_gradients_finite_at_prior_corners(knee):
    """grad(pta_logL) is finite at every corner of the production prior box.

    NaN gradients characteristically surface at parameter-space corners
    (exponent bounds like κ = 0 or δ = 0, band-edge bend frequencies — where
    ``d/dκ x^κ = x^κ ln x`` terms meet saturating factors), not at friendly
    interior points, so the sweep runs all 2^n corners of the
    :func:`turnover_priors` box NUTS actually samples in.  Liveness (nonzero
    shape-parameter gradients) is asserted at the interior default only: at
    corners like ``log10_A = -18`` the GWB term legitimately vanishes against
    the white noise and its gradients may be exactly zero.
    """
    from itertools import product

    from jaxpint.bayes.samplers import turnover_priors

    spec = TurnoverKneeSpectrum() if knee else TurnoverSpectrum()
    inj = CURNInjector(N_COMP, T_SPAN, spectrum=spec)
    gp, pps, config = _config_and_params(inj, correlated=False)
    grad_fn = jax.jit(
        jax.grad(lambda v: pta_logL(gp.with_values(v), pps, config))
    )

    g0 = np.asarray(grad_fn(gp.values))
    assert g0.shape == (6 if knee else 4,)
    assert np.isfinite(g0).all()
    shape_names = (
        ("gwb_lfb", "gwb_lfk", "gwb_kappa", "gwb_delta")
        if knee
        else ("gwb_lf0", "gwb_kappa")
    )
    for name in shape_names:
        assert g0[gp.param_index(name)] != 0.0

    bounds = {
        name: (float(d.low), float(d.high))
        for name, d in turnover_priors(knee=knee).flat.items()
    }
    for corner in product(*(bounds[n] for n in gp.names)):
        g = np.asarray(grad_fn(jnp.asarray(corner)))
        assert np.isfinite(g).all(), (
            f"non-finite gradient at prior corner {dict(zip(gp.names, corner))}"
        )
...
@pytest.mark.parametrize("knee", [False, True], ids=["turnover", "turnover_knee"])
def test_turnover_priors_match_registered_names(knee):
    """turnover_priors covers exactly what the injector registers."""
    from jaxpint.bayes.samplers import turnover_priors

    spectrum = TurnoverKneeSpectrum() if knee else TurnoverSpectrum()
    inj = CURNInjector(N_COMP, T_SPAN, spectrum=spectrum)
    gp = inj.register_params(GlobalParams.empty())
    spec = turnover_priors(knee=knee)
    assert spec.owned_names() == set(gp.names)
    # Shape bounds mirror enterprise_extensions' production blocks.
    d = spec.flat["gwb_kappa"]
    assert float(d.low) == 0.0 and float(d.high) == 7.0
    if knee:
        assert float(spec.flat["gwb_lfb"].low) == -9.3
        assert float(spec.flat["gwb_delta"].high) == 0.0
    else:
        assert float(spec.flat["gwb_lf0"].low) == -9.0
    # Custom prefix form.
    spec2 = turnover_priors(prefix="crn_", knee=knee)
    assert "crn_kappa" in spec2.owned_names()
