...
# ---------------------------------------------------------------------------
# Enterprise-generated goldens (committed data; see tools/gen_enterprise_goldens.py)
# ---------------------------------------------------------------------------

_ENTERPRISE_GOLDENS = Path(__file__).parent / "data" / "enterprise_goldens.json"


@functools.lru_cache(maxsize=None)
def _load_enterprise_goldens() -> dict:
    return json.loads(_ENTERPRISE_GOLDENS.read_text())


def enterprise_golden(name: str, *, params: dict, grid: dict) -> np.ndarray:
    """Committed reference values generated by enterprise itself.

    Cached reference-stack output, not an in-repo transcription — enterprise
    is needed only to regenerate the file (run
    ``tools/gen_enterprise_goldens.py``), never to run the tests; the live
    guard that the frozen values track current enterprise is
    tests/enterprise_checks/.  ``params`` and ``grid`` are what the calling
    test *expects* the golden to have been generated with; they are asserted
    against the generation record stored in the JSON, so a test and its
    golden cannot silently disagree (checked metadata instead of comments).
    """
    entry = _load_enterprise_goldens()[name]
    assert entry["params"] == params, (
        f"golden {name!r} was generated with {entry['params']}, but the test "
        f"expects {params}; rerun tools/gen_enterprise_goldens.py after "
        f"reconciling the spec"
    )
    assert entry["grid"] == grid, (
        f"golden {name!r} was generated on grid {entry['grid']}, but the test "
        f"expects {grid}; rerun tools/gen_enterprise_goldens.py after "
        f"reconciling the spec"
    )
    return np.asarray(entry["values"], dtype=np.float64)
